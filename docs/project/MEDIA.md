...
| ~~`docs/media/hero.mp4`~~ (+`hero-poster.png`) — **removed 2026-09-02**: the phone app is not shipped, so no public surface shows it (user call). The rig stays: `e2e/demo/record-hero.sh` (desk take `record-hero-desk.sh` + phone take `record-hero-phone.sh` inside its away-wait, cut by `cut-hero.sh`) needs a live Claude Code login AND a booted iOS simulator with a dev build of the phone app | was the 0.19 hero — one serve, one terminal session: the desk hands work to an agent, a phone closes an issue while the chair is empty, the desk comes back to the scrollback and the board. Re-shoot and re-add when the phone app ships. **The no-public-surface half of that call was reversed on 2026-09-16** (user): the landing and the three READMEs now carry recorded phone stills marked *beta* (the `phone-*.png` row above, `docs/project/FACT_LEDGER.md` section 3a). This clip stays removed on its own merits — it needs a live login and a booted simulator, and its climax is a continuity claim (`docs/runbooks/release-video.md`, G2) |
...
| `docs/media/phone.mp4` (+`phone.gif`, `phone-poster.png`, and the `.ko` / `.ja` takes of each) | Playwright `mobile/clip/phone-clip.spec.ts` via `mobile/clip.config.ts`, cut by `mobile/clip/export-phone-clip.sh` (`make media-phone-clip`, `GADAK_MEDIA_LOCALE=ko|ja` for the other two; outside the `make media` aggregate, same reason as `media-sprint`) | landing phone exhibit + the READMEs' phone section — one ~28s take of the app's own viewport at 402x874, laid out there and painted at 3x by zooming the document (Playwright's video is a css-pixel capture — `mobile/clip.config.ts` says what that cost before it was understood), published at 804x1748 because the landing caps the element at 300 css px. No bezel and no fabricated status bar. The take reads first and then writes: the list with its active-sprint line, the list scrolled, the palette open in the heading (the beat that says the tab bar is gone), `NMB-110` typed into it, that issue's Fields block and attachment grid — then the status moved to In Progress, a comment typed, a photo attached, both landing in the thread, and back to the list it opened on. **The writes are real.** The rig serves `gadak demo --writable` (GDK-1959), so the snapshot is a tracker of its own and every write reaches an origin; the take reads each result back out of the app and the exporter refuses a take whose ledger is empty, because a film of someone tapping disabled controls looks exactly like a film of someone writing. The landing plays the whole mp4; the READMEs show a gif of the writing half only, and link to `gadak.dev/#phone` for the rest. That is not a preference — measured 2026-09-16 against GitHub's own `/markdown` endpoint, a `<video>` tag renders as an empty paragraph, `![](x.mp4)` and `<img src=x.mp4>` both render a broken image, and a bare URL renders a link; GitHub's blob view answers `viewable:false` for an mp4, so even the file page only offers a download. The one URL that does get a player is `user-attachments/assets/<uuid>`, which exists only by dragging a file into an issue box — not reproducible from this repo, and dead on every renderer that is not GitHub. The gif is the writing half rather than the whole take for a second, independent reason: the whole of it at a frame rate that keeps typed prose legible does not fit 4 MB (measured: 28.7s came out at 4.59 MB at fps 9, and the ladder's next rung is where the letters die). The take names its own cut point. The export opens on the frame the take closes on, measured by SSIM against its own last frame, so the landing's `loop` has no seam and a take that never came home is refused |
| ~~`docs/media/phone-list.png` · `phone-palette.png` · `phone-detail.png`~~ (+ the `.ko.png` / `.ja.png` takes of each) — **removed 2026-09-16**, the day after they landed: the user's call is a clip rather than stills ("나는 영상이 들어가는게 좋아 이미지 말고"), and the clip above walks the same three surfaces in the order a person reaches them. The rig stays — `mobile/media/phone.spec.ts` via `mobile/media.config.ts` (`make media-phone`) is the frame-by-frame review camera and the owner of `GADAK_MEDIA_LOCALE` for the phone tree, which `clip.config.ts` imports | were the landing phone exhibit and the READMEs' phone section |
...
| phone (beta) | `phone.mp4` (+`phone-poster.png`), per locale via `mediaFor` | video — **the one exception to the motion rule, and it is a user call** (2026-09-16, GDK-1955). The rule sent a phone screen to a still because it reads from one frame; what a still could not say is the part that is new, which is that the phone is the same cache *navigated* — list, palette, key, issue, back. Recorded at the phone's own size rather than cropped from a desktop capture, which is what `docs/runbooks/release-video.md` found unreadable at 1:1, and capped at 300 px on the page: the take is 2.17:1. It autoplays muted and loops, like the flagship. The exhibit says *beta* and promises no download: there is no public TestFlight and no store listing (GDK-958) |
...
GADAK_MEDIA_LOCALE=ko make media-phone-clip # -> phone.ko.mp4, phone.ko.gif, phone-poster.ko.png
