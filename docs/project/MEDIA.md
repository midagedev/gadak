...
| `docs/media/phone-list.png` · `phone-palette.png` · `phone-detail.png` (+ the `.ko.png` / `.ja.png` takes of each) | Playwright `mobile/media/phone.spec.ts` via `mobile/media.config.ts` (`make media-phone`, `GADAK_MEDIA_LOCALE=ko|ja` for the other two; outside the `make media` aggregate, same reason as `media-sprint`) | landing phone exhibit + the READMEs' phone section — the app's own viewport at 402x874, device scale 3, no bezel and no fabricated status bar. `phone-list` is the list with the active-sprint line, `phone-palette` the palette open in the heading (the frame that says the tab bar is gone), `phone-detail` NMB-110 with the Fields and Attachments sections. A fourth frame, the sprint list, was shot and dropped on 2026-09-16: no public surface spends a picture on it and the screen is mostly empty frame at this viewport |
...
| phone (beta) | `phone-list.png`, `phone-detail.png`, `phone-palette.png`, per locale via `mediaFor` | stills — a phone screen reads from one frame, so the motion rule sends it to a still. Shot at the phone's own size rather than cropped from a desktop capture, which is what `docs/runbooks/release-video.md` found unreadable at 1:1. The exhibit says *beta* and promises no download: there is no public TestFlight and no store listing (GDK-958) |
...
GADAK_MEDIA_LOCALE=ko make media-phone     # -> phone-list.ko.png, phone-palette.ko.png, phone-detail.ko.png
