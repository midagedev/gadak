...
| `docs/media/phone.mp4` (+`phone.gif`, `phone-poster.png`, and the `.ko` / `.ja` takes of each) | Playwright `mobile/clip/phone-clip.spec.ts` via `mobile/clip.config.ts`, cut by `mobile/clip/export-phone-clip.sh` (`make media-phone-clip`, `GADAK_MEDIA_LOCALE=ko|ja` for the other two; outside the `make media` aggregate, same reason as `media-sprint`) | landing phone exhibit + the READMEs' phone section — one ~16s take of the app's own viewport at 402x874, laid out there and painted at 3x by zooming the document (Playwright's video is a css-pixel capture — `mobile/clip.config.ts` says what that cost before it was understood), published at 804x1748 because the landing caps the element at 300 css px. No bezel and no fabricated status bar: the list with its active-sprint line, the list scrolled, the palette open in the heading (the beat that says the tab bar is gone), `NMB-110` typed into it, that issue's Fields block and attachment grid, and back to the list it opened on. The landing plays the mp4 and the READMEs show the gif, because GitHub strips `<video>` from markdown. The export opens the cut on the frame the take closes on, measured by SSIM against its own last frame, so the landing's `loop` has no seam and a take that never came home is refused |
