...
that directory is a cache of the site. On a gadak origin
...
gadak and you have lost nothing but a cache of your Jira. On a gadak origin, the
...
was never in it; a workspace whose origin is gadak's own tracker's data is that SQLite file.
...
- **Linear** is a different tracker, and also a gadak source: a `"linear"`
  ...
  anyone for permission — it is a mirror, not a migration. A gadak-origin
