...
| `started_at` | Timestamp of the oldest changelog transition whose target category is `inprogress` — or `created_at` when the issue was **born in progress** (its oldest status entry leaves an in-progress category, or it has no status history on an origin that supplies one and sits in progress now). On an origin that supplies no history (Linear) the issue's own `startedAt` stamp fills the column instead — a hint consulted only under `NoHistory`, never for an origin with a changelog. NULL when there is neither history nor a stamp: a guessed start would be a confident wrong number |
| `cycle_hours` | `resolved_at` minus `started_at` in hours, stored only while the issue is `done` now and the span is positive; NULL otherwise. On Linear both ends come from the issue's own stamps (`startedAt` → `completedAt`, or `canceledAt` on a canceled issue — canceled maps to `done`), under the same done-now and positive-span guards |
| `carryover_count` | How many times the issue was carried into a sprint after the first: the number of distinct sprints it has ever entered, minus one, floored at 0. Read from the changelog's `sprint` rows, whose field id is per-site and is normalised to `sprint` on the way in. Two origin shapes are read by one rule — Jira Cloud keeps the whole membership as a growing comma list (`12` → `12, 13`), the built-in tracker states the single sprint moved into (`12` → `13`) — so a removal and a re-add of a sprint already visited both count nothing. **NULL, never 0, on an origin that supplies no changelog** (Linear): never carried and cannot be read are different answers |
| `first_sprint_id` | The first sprint the issue entered, in the `sprints(source_id, id)` space; NULL when it never entered one |
| `first_sprint_at` | Timestamp of that first entry. The scope-creep question — was this added after the sprint began? — is this stamp against that sprint's `start_at`, which is why the stamp is stored and not only the count |
| `blocked_hours` | Time the issue has spent flagged, in hours: every **closed** flag interval summed (a set followed by a clear). Read from the changelog's `flagged` rows — Jira's Flagged checkbox, whose field id is per-site and is normalised to `flagged` on the way in, sprint's exact story. A set while already up does not restart the interval; a clear with nothing up closes nothing (the set predates the changelog's horizon, and an unmeasurable interval is skipped, not guessed). **0 means never flagged; NULL, never 0, wherever the flag history cannot be read** — never blocked and cannot be read are different answers. NULL has two sources: an origin that supplies no changelog (Linear), and a mirror that upgraded into v51, whose older rows still carry the site's own field id. The migration recognises no such id — a checkbox has no sprints-like table to join it against — so it derives the column only for the rows it can read and leaves the rest NULL rather than claiming 0. The next sync of an issue rewrites its changelog under the stable name; `gadak sync --full` does the whole mirror in one pass |
| `blocked_since` | The open flag interval's start — when the issue was last flagged and never unflagged; NULL when no flag is up. A stamp, not a live duration: the flag's age is the reader's `now − blocked_since`, which is why the stamp is the stored column |
