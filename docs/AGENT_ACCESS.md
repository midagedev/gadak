...
mirror — use **`gadak api`**. (Watchers in that list is Jira's site-side
subscribers on an issue, never mirrored. gadak's own **watches** are the
opposite direction — your local follows, kept in `local.db` and exported with
favorites and recents; the site never hears about them.) It sends the request
with the workspace's stored
...
**Not on MCP.** `gadak api` is CLI-only. MCP stays ten tools
(`gadak_query`, `gadak_search`, `gadak_issue`, `gadak_status`, `gadak_show`,
`gadak_retro`, `gadak_sprint`, `gadak_recents`, `gadak_ui_tokens`, `gadak_ui_set`) with no writes to the mirror or to Jira and
...
`gadak_show`, `gadak_retro`, `gadak_sprint`, `gadak_recents`, and the
`gadak_ui_tokens` / `gadak_ui_set` settings pair.
