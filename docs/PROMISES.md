...
Eleven things gadak asks you not to take on trust: one sentence each, and one
command you can run in a clone of this repository — a Go toolchain for six
...
Your own Atlassian site; Linear (`api.linear.app` GraphQL and a signed PUT
to `uploads.linear.app`); a pairing home serve; user-invoked `gh`; a
one-shot, user-invoked `gadak dashboards lib add <url>` download to the URL
the user typed. The `atlassian.net` strings below are placeholders in help
text and comments, and `cdn.jsdelivr.net` is the example URL in that
command's own help. `developer.microsoft.com`, `x.com`, and `github.com` are
desktop Help-menu / help-text strings, not hosts the HTTP client calls;
`https://github` is the regex literal `https://github\.com` truncated by
this grep.
...
**3. The mirror is an ordinary SQLite file, readable without gadak.**
...
**4. The mirror cannot be written through the paths an agent reads.**
...
**5. `gadak serve` refuses a non-loopback bind unless you ask for one.**
...
**6. A hostile web page cannot post through gadak, or rebind a DNS name at it.**
...
**7. A snapshot you hand to someone else cannot carry your API token.**
...
**8. On the built-in tracker, the origin is one ordinary SQLite file, readable without gadak.**
...
**9. Deleting `gadak.db` loses nothing you made.**
...
**10. What you type in gadak's terminal never touches disk.**
...
**11. Only a terminal-scoped pairing token can open a shell.**
