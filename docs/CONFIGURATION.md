...
| `confluence` | object or absent | absent = wiki mirror off. `gadak init --local` writes the block scoped to `LOC`; pairing writes it with `spaces` empty (every team space of the home origin — anything but personal) | Settings → Sources / `gadak config set confluence` | Next Confluence pass |
