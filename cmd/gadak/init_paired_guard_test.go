...
func assertPairedOriginUntouched(t *testing.T, dir string) {
	...
	if cfg.HasBuiltInOrigin() {
		t.Fatal("refused init must not flip the workspace to built-in")
	}
...
// TestInitPairedRefusesBuiltIn is GDK-452: `gadak init --standalone` on a
// profile bound to a remote serve must not replace the origin with an empty
// local persist.
func TestInitPairedRefusesBuiltIn(t *testing.T) {
...
// paired origin (origin.pairedRemote excludes builtIn); init --standalone
// must not pick up the paired-origin refusal.
func TestInitBuiltInHomeRoutingNotRefused(t *testing.T) {
	pairingHome(t)
	_, _, err := captureErr(t, func() error {
