...
func TestDecidePortBusy_SameProfileGadak(t *testing.T) {
	probe := func(port string) gadakProbe {
...
func TestDecidePortBusy_OtherProfileOrProcess(t *testing.T) {
	cases := []struct {
		...
		{
			name:    "other gadak profile",
			profile: "default",
			probe:   gadakProbe{IsGadak: true, Profile: "work"},
			wantSub: `gadak profile "work"`,
		},
		{
			name:    "empty vs named",
			profile: "",
			probe:   gadakProbe{IsGadak: true, Profile: "demo"},
			wantSub: `gadak profile "demo"`,
		},
		{
			name:    "not gadak",
			profile: "work",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := decidePortBusy("127.0.0.1:7777", false, tc.profile, func(string) gadakProbe {
				return tc.probe
			})
...
func TestDecidePortBusy_PinnedAddrFails(t *testing.T) {
	// Explicit --addr: never fall back; surface gadak identity when present.
	got := decidePortBusy("127.0.0.1:9001", true, "work", func(string) gadakProbe {
		return gadakProbe{IsGadak: true, Profile: "other"}
	})
	if got.Action != portActionFail {
		t.Fatalf("action = %v, want fail", got.Action)
	}
	if !strings.Contains(got.ErrDetail, "gadak") {
		t.Fatalf("ErrDetail = %q, want gadak mention", got.ErrDetail)
	}

	got2 := decidePortBusy("127.0.0.1:9001", true, "work", func(string) gadakProbe {
		return gadakProbe{}
	})
...
func TestDecidePortBusy_PinnedSameProfileStillFails(t *testing.T) {
	// Pin means "this address or error" — even same-profile gadak must not
	// silently open-existing when the user forced the port? Spec rule 3:
	// "명시 addr였으면 폴백하지 않는다" and "점유자가 gadak로 식별되면 에러 메시지에 부가".
	// Open-existing (rule 1) only applies to default addr. Pinned → fail.
	got := decidePortBusy("127.0.0.1:7777", true, "work", func(string) gadakProbe {
		return gadakProbe{IsGadak: true, Profile: "work"}
	})
	if got.Action != portActionFail {
...
func TestBindListen_OpenExistingSameProfile(t *testing.T) {
	...
	mux.HandleFunc("/api/v1/issues/sync/progress/", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("X-Gadak-Profile", "work")
