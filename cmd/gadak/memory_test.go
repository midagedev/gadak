...
// builtInMemory is the memory-verb fixture: a built-in workspace (the
// in-process origin, no network) and nothing else — memory add creates the
// first page itself.
func builtInMemory(t *testing.T) {
	t.Helper()
	home := t.TempDir()
...
func TestMemoryAddSearchRoundtrip(t *testing.T) {
	...
	if space != origin.DefaultSpaceKey {
		t.Errorf("built-in default space = %q, want the seeded %q", space, origin.DefaultSpaceKey)
	}
