...
// and whether the built-in origin died with it.
...
func removeWorkspace(invoked string, args []string) error {
	...
	destroyOrigin := fs.Bool("destroy-origin", false, "built-in only: also destroy this profile's persist, the only copy of that tracker")
	...
	if doc.OriginDestroyed {
		fmt.Printf("destroyed built-in origin: %s\n", res.Persist)
	} else if res.BuiltIn {
		fmt.Println("no built-in persist was present; no origin data existed to destroy")
	} else {
