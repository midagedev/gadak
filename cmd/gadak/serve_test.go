...
func TestServeScopeLogBuiltInOmitsAccount(t *testing.T) {
	// GDK-464
	...
	if strings.Contains(st, "this account") {
		t.Fatalf("built-in serve line still mentions an account: %q", st)
	}
	if st != "syncing "+origin.DefaultProjectKey {
		t.Fatalf("built-in serve line = %q", st)
	}
...
func TestBuiltInConfigDoesNotNeedAdvertiseFile(t *testing.T) {
	home := t.TempDir()
	...
	if _, err := os.Stat(filepath.Join(cfg.Directory(), "serve-origin.json")); !os.IsNotExist(err) {
		t.Fatal("built-in config must not write serve-origin.json")
	}
}
