...
// current GADAK_HOME for the named profile ("" = default). Credentials are
...
func TestProfilesJSONThreeRowsAndActive(t *testing.T) {
	...
	// ② GADAK_PROFILE=work → active work, only that row active:true
...
func TestProfilesDoesNotCreateMissingMirror(t *testing.T) {
	...
	// Named profile with config only — no gadak.db.
	...
	if _, err := os.Stat(filepath.Join(dir, "gadak.db")); !os.IsNotExist(err) {
		t.Fatalf("profiles must not create gadak.db; stat err=%v", err)
	}
