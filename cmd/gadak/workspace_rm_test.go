...
// seedPersist plants a built-in persist (origin/issuetap.db) inside dir.
...
func TestWorkspaceRMBuiltInRequiresDestroyOrigin(t *testing.T) {
	rmTestHome(t)
	...
	if err == nil {
		t.Fatal("built-in rm with --yes only: expected refusal")
	}
	msg := err.Error()
	for _, want := range []string{"--destroy-origin", persist, "only copy"} {
		if !strings.Contains(msg, want) {
			t.Errorf("built-in refusal should mention %q, got:\n%s", want, msg)
		}
	}
	...
	if !strings.Contains(out, "standalone") {
		t.Errorf("success output should name the built-in origin destruction, got:\n%s", out)
	}
}

func TestWorkspaceRMBuiltInNoPersistNeedsOnlyYes(t *testing.T) {
	rmTestHome(t)
	...
	if err != nil {
		t.Fatalf("built-in without persist should remove with --yes alone: %v", err)
	}
	if _, err := os.Stat(dir); !os.IsNotExist(err) {
		t.Fatalf("profile dir should be gone, stat err = %v", err)
	}
...
func TestWorkspaceRMJSONShape(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("built-in json rm: %v", err)
	}
	doc = nil
	if err := json.Unmarshal([]byte(out), &doc); err != nil {
		t.Fatalf("stdout is not one JSON object: %v\n%s", err, out)
	...
	if len(doc) != 3 || doc["removed"] != "s3" || doc["kind"] != "standalone" || doc["origin_destroyed"] != true {
		t.Errorf("built-in json shape wrong: %v", doc)
	}
}
