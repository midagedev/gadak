...
func TestPrettyOpenURL(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:     "loopback bind + loopback resolve → gadak.localhost",
			bindHost: "127.0.0.1",
			port:     "7777",
			lookup:   loopbackLookup,
			want:     "http://gadak.localhost:7777",
		},
		{
			name:     "localhost bind + loopback resolve → gadak.localhost",
			bindHost: "localhost",
			port:     "7777",
			lookup:   loopbackLookup,
			want:     "http://gadak.localhost:7777",
		...
		{
			name:     "::1 bind + loopback resolve → gadak.localhost",
			bindHost: "::1",
			port:     "7777",
			lookup:   loopbackLookup,
			want:     "http://gadak.localhost:7777",
		...
		{
			name:     "empty bind host + loopback resolve → gadak.localhost",
			bindHost: "",
			port:     "7777",
			lookup:   loopbackLookup,
			want:     "http://gadak.localhost:7777",
...
func TestBrowseAddrUsesPrettyHost(t *testing.T) {
	...
	// is healthy; injected path is covered above. When gadak.localhost is not
	// all-loopback, browseAddr must still return a usable URL.
	got := browseAddr("127.0.0.1:7777")
	if got != "http://gadak.localhost:7777" && got != "http://127.0.0.1:7777" {
		t.Fatalf("browseAddr = %q, want gadak.localhost or 127.0.0.1 form", got)
	}
}
