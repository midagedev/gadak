...
var helps = map[string]cmdHelp{
	...
	"mcp": {
		...
		seeAlso: []string{"scry skill install", "scry sql", "scry issue", "scry status", "scry profiles"},
	},
	"skill": {
		summary: "install the Claude Code skill (schema + query patterns; no MCP process)",
		usage:   "scry skill install [client] [--project] [--dir <path>] [--print] [--force]",
		options: []helpOption{
			{name: "project", desc: "install into ./.claude/skills/scry/ under the current directory"},
			{name: "dir", desc: "install into PATH/scry/SKILL.md (overrides default and --project)"},
		...
		seeAlso: []string{"scry mcp install", "scry sql", "scry issue"},
	},
	"snapshot": {
