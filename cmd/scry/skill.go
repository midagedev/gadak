...
// scry skill install [client] — install the embedded Claude Code skill so
// agents that prefer skills over MCP get schema/query knowledge without a
// server process. Source is go:embed'd (scry.SkillMarkdown); brew installs
...
func printSkillHelp() {
	fmt.Print(`scry skill — install the Claude Code skill (schema + query patterns)

Usage:
  scry skill install [client] [--project] [--dir <path>] [--print] [--force]

Clients:
  claude   Claude Code skill (default). Installs SKILL.md under skills/scry/

Other agents (cursor, codex, …) are not supported by this command yet —
use scry mcp install <client>, or copy skills/scry/SKILL.md from the repo
yourself.

Options:
  --project   install into ./.claude/skills/scry/ (current working directory)
  --dir PATH  install into PATH/scry/SKILL.md (overrides default and --project)
...
Install locations (when --dir is omitted):
  default     ~/.claude/skills/scry/SKILL.md
  --project   .claude/skills/scry/SKILL.md under the current directory

Examples:
  scry skill install
  scry skill install claude
  scry skill install --print
  scry skill install --project
  scry skill install --dir /tmp/skills-preview --print
  scry skill install --force

See also: scry mcp install, docs/AGENT_SETUP.md, docs/MCP.md
`)
}
...
func cmdSkill(args []string) error {
	...
	return usageError("skill", "usage: scry skill install [client] [--project] [--dir <path>] [--print] [--force]")
}
...
func cmdSkillInstall(args []string) error {
	...
	for i := 0; i < len(args); i++ {
		...
		case a == "--dir":
			if i+1 >= len(args) {
				return fmt.Errorf("--dir requires a path\nrun \"scry skill install --help\" for examples")
			}
			i++
			dirFlag = args[i]
		...
		case strings.HasPrefix(a, "-"):
			return fmt.Errorf("unknown flag %s\nrun \"scry skill install --help\" for examples", a)
		default:
	...
	if len(positionals) > 1 {
		return usageError("skill install", "usage: scry skill install [client] [--project] [--dir <path>] [--print] [--force]")
	}
	if len(positionals) == 1 {
	...
	if client != "claude" {
		return fmt.Errorf("unknown client %q — only Claude Code skills are supported (client: claude)\n"+
			"for other agents use `scry mcp install <client>`, or copy skills/scry/SKILL.md yourself\n"+
			"run \"scry skill install --help\" for examples", client)
	}
	...
	return installSkill(os.Stdout, scry.SkillMarkdown(), dest, force, printOnly)
}

// resolveSkillDest picks the SKILL.md path.
// --dir wins (dir/scry/SKILL.md); else --project → cwd/.claude/skills/scry/;
// else ~/.claude/skills/scry/SKILL.md.
func resolveSkillDest(project bool, dirFlag string) (string, error) {
	if dirFlag != "" {
		...
		return filepath.Join(abs, "scry", "SKILL.md"), nil
	}
	if project {
		cwd, err := os.Getwd()
		if err != nil {
		...
		return filepath.Join(cwd, ".claude", "skills", "scry", "SKILL.md"), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return "", fmt.Errorf("home directory: %w", err)
	}
	return filepath.Join(home, ".claude", "skills", "scry", "SKILL.md"), nil
}
...
func installSkill(w io.Writer, content []byte, dest string, force, printOnly bool) error {
	if len(content) == 0 {
		return fmt.Errorf("embedded skill is empty — this binary was built without skills/scry/SKILL.md")
	}
	...
	if printOnly {
		fmt.Fprintf(w, "source:  embedded skills/scry/SKILL.md\n")
