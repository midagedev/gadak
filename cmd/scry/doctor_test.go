...
func TestDoctorNoMirrorSucceeds(t *testing.T) {
	...
	if !strings.HasPrefix(strings.TrimSpace(out), "# scry doctor") {
		t.Fatalf("missing safety banner:\n%s", out)
	}
	// Must not create a database just by diagnosing.
	if _, err := os.Stat(filepath.Join(home, "scry.db")); !os.IsNotExist(err) {
		t.Fatalf("doctor must not create scry.db when missing; stat err=%v", err)
	}
}
...
func TestDoctorDemoDBCounts(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(home, "scry.db"), raw, 0o600); err != nil {
...
func TestDoctorJSONMatchesHumanFields(t *testing.T) {
	...
	if rep.ScryVersion == "" || rep.GoVersion == "" || rep.OS == "" || rep.Arch == "" {
...
func TestTildeHome(t *testing.T) {
	...
	got := tildeHome(filepath.Join(home, ".scry", "scry.db"))
	if got != "~/.scry/scry.db" && got != `~\.scry\scry.db` {
