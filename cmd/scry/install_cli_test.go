...
func TestInstallCLINewSymlink(t *testing.T) {
	...
	root := t.TempDir()
	source := filepath.Join(root, "bin", "scry")
	...
	if !strings.Contains(out, "next: scry mcp install claude") {
...
func TestInstallCLIConflictAndForce(t *testing.T) {
	...
	root := t.TempDir()
	source := filepath.Join(root, "new-scry")
...
func TestInstallCLIPrintNoOp(t *testing.T) {
	...
	root := t.TempDir()
	source := filepath.Join(root, "scry")
	if err := os.WriteFile(source, []byte("x"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := os.Lstat(filepath.Join(dir, "scry")); !os.IsNotExist(err) {
...
func TestPathExportLineZsh(t *testing.T) {
	line, rc := pathExportLine("/opt/scry/bin", "/bin/zsh")
	...
	if !strings.Contains(line, `export PATH="/opt/scry/bin:$PATH"`) {
