...
// scry mcp — stdio MCP server for clients without a shell.
...
import (
	...
	"github.com/midagedev/scry/internal/mcp"
)
...
func cmdMCP(args []string) error {
	...
	if _, err := os.Stat(path); err != nil {
		mcp.Logf("no mirror at %s — tools will error until you run `scry init && scry sync`", path)
	}
