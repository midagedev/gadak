<script lang="ts">
  ...
  /** Lift the menu onto document.body so a transformed/overflow ancestor
   *  cannot trap its stacking context under the next detail section. */
  ...
  function chipLabel(v: string): string {
    if (kind === 'date') return calendarDay(v, 'date') ?? v
    return v
  }

  function parseNumberInput(raw: string): string | null | 'invalid' {
    const s = raw.trim()
    if (!s) return null
  ...
  function userSelected(c: Cand): boolean {
    if (field === 'development_test_assignee') {
      return c.email != null && c.email === issue.development_test_assignee_email
    }
    return values.includes(c.display_name)
  }

  async function pickUser(c: Cand | null) {
    busy = true
    const patch: Partial<IssueLite> =
      field === 'development_test_assignee'
        ? {
  ...
  async function commitScalar() {
    if (kind === 'number') {
      const parsed = parseNumberInput(draftText)
      ...
      busy = true
      const ok = await write.setField(key, field, parsed, {
        [field]: parsed,
      } as Partial<IssueLite>)
      busy = false
      if (ok) open = false
    ...
    const ok = await write.setField(key, field, value, {
      [field]: value,
    } as Partial<IssueLite>)
    busy = false
    if (ok) open = false
...
<div class="relative inline-block w-full">
  <button
    ...
    aria-expanded={open}
    data-testid="field-editor-trigger"
  ...
  >
    ...
    {:else}
      <span class="flex min-w-0 flex-wrap gap-1">
        {#each values as v (v)}
          <span class="max-w-full break-words rounded px-1.5 py-0.5 {chipClass(v)}">{chipLabel(v)}</span>
        {/each}
      </span>
    {/if}
    <svg
