...
{#if all.length === 0}
  <p class="text-[12px] text-text-muted italic">코멘트 없음</p>
{:else}
      ...
      <li class="group anim-enter" class:opacity-60={c.comment_id.startsWith('temp-')}>
        <div class="mb-1 flex items-center gap-2">
          ...
          <span class="text-[12px] font-medium text-text-primary">
            {c.author ?? c.author_email ?? '알 수 없음'}
          </span>
          ...
          {#if me.authed && c.author_account_id && !c.comment_id.startsWith('temp-')}
            <button
              ...
              onclick={() => reply(c)}
              title="이 코멘트에 답글">답글</button
            >
          {/if}
        </div>
        <div class="ml-[28px] text-[13px] leading-relaxed text-text-secondary">
          <AdfContent node={c.raw_body} {issueKey} {attachments} fallback={c.body} emptyLabel="(빈 코멘트)" />
        </div>
      </li>
    {/each}
  </ol>
