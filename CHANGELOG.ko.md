...
**그 밖에: 회고 화면, 되돌릴 수 있는 코멘트, 실물 크기의 첨부, 아웃바운드
호출 하나 감소, 그리고 여태 짐작에 맡겨 두었던 답 둘.** `gadak retro`의 문서는 오지 않은 화면을 위해
...
앱이 없는 Jira 사이트가 그렇습니다 ([GDK-529]). 세션 명부가 셸마다 무슨 일을 하는지
말합니다. 셸이 스스로 거는 창 제목 — Claude Code 는 하는 일이 바뀔 때마다 그것을
고쳐 씁니다 — 이 세션 이름 아래 부제로 붙습니다. 이름은 사람이 고른 그대로
남습니다. 파서는 하나입니다: 제목의 BEL 종결자와 진짜 BEL 을 가르려면 그 문자열이
어디서 끝나는지 이미 알아야 했던 그 OSC 스캐너입니다. 제목은 '사람을 부른다' 는
신호를 절대 올리지 않고, 내용이 셸이 실행한 것이 찍기로 한 무엇이든이므로 길이가
제한되고 제어문자가 걷힙니다. 제목이 없는 세션은 두 번째 줄을 아예 그리지
않습니다 ([GDK-1389]). 시크릿 스캐너가 원래 하겠다고 한 만큼을
봅니다. `internal/secretscan` 은 자격증명 일곱 형을 들고 있고 나가는 산출물은
전부 그 일곱에 대해 검사받는데, 그 표를 레포로 겨누는 스크립트는 둘만 그렙하고
있었습니다 — 진짜 `ghp_` 토큰이 픽스처에 커밋돼도 게이트가 초록이었다는 뜻입니다.
이제 Slack·GitHub 과 Authorization 헤더 두 형도 트리를 훑습니다. 그중 셋은
`(?i)` 나 `\b` 에 이식 가능한 철자가 없어 POSIX ERE 로 다시 적고 일치를 동작으로
단언합니다. 걸린 줄은 어떤 형이 맞았는지 이름을 답니다. 마지막으로 PEM 이 남은
자리를 메웠습니다. 트리에 정당한 일치가 있는 유일한 형이라 파일별 면제가 필요했고,
그 목록은 두 읽는 쪽이 함께 보는 파일 하나에 있습니다 — 한 줄은 정확한 경로와
'이 파일이 키 헤더를 담아도 되는 이유'이고, 이유 없는 줄은 파싱에서 거절당하며,
그 파일이 더는 해당 형을 담고 있지 않으면 스캔이 빨개지면서 지울 줄을 스스로
짚습니다 ([GDK-1110], [GDK-1797]). 설정 창은 레지스트리가 한 번도 가져 본 적
없는 게이트를 얻었습니다. 거기서 표면에 따라 갈리는 것은 탭 단위이고 그것을
소유한 목록도 이미 하나였지만, 그 목록이 '데스크톱 서버가 실제로 필요한 탭'
과 여전히 맞는지 재는 것이 없었고, 탭 안의 행이 자기만의 분기를 기르는 것을
막는 것도 없었습니다. 이제 양쪽을 각자의 소유자에서 읽습니다. 그 목록을 만들다
전제 하나도 정리됐습니다 — `hosted` 는 그 창에 아예 닿지 않습니다. 스냅샷에서
정확히 거짓이 되는 조건 아래에 걸려 있기 때문입니다. 설정의 표면은 셋이 아니라
둘입니다 ([GDK-9]).
...
([GDK-1793]). 열 사이의 그립을 팔레트에서 잡을 수 있습니다 — '사이드바 너비 조절'·'이슈
목록 너비 조절' 이 화살표 키가 살아 있는 채로 그립에 포커스를 줍니다. GDK-759 이후로 그립은
키보드가 다 되는 진짜 버튼이었지만 탭 순서로는 150번째였습니다. 도달을 재는 것이 없었으므로,
열 목적지를 덮던 팔레트 감사가 드래그 축도 같은 방식으로 덮습니다 ([GDK-1796]).
토큰에 대한 주의 — 모르는 이름, 16진수가 아닌 값 — 는 팔레트마다 한 줄이 아니라 다시 한
줄입니다. 쓰기 경로는 이미 접고 있었는데 `ui.warnings` 를 채우는 읽기 경로가 안 접고
있었습니다 ([GDK-769]). fonts 축은 '토큰 이름' 자리에 자기 토큰을 직접 적습니다 — 저장된
오버라이드를 돌려주는 명령을 가리키지 않습니다. 그걸 잡으라고 있던 게이트는 명령이
언급됐는지만 보고 있었습니다 ([GDK-769]). 페어링 워크스페이스에 없는 Atlassian 계정을
들먹이던 문장 둘도 그만뒀습니다 — serve 의 시작 줄과 sync 의 범위 라벨이 이제 '페어링한
serve 가 들고 있는 프로젝트' 라고 말합니다. 같은 오독의 마지막 자리였습니다 ([GDK-1793]).
짐작에 기대고 있던 답 둘이 이제 도구에서 나옵니다. `gadak doctor` 가 버전 옆에
`binary` 줄을 찍습니다 — 심볼릭 링크를 푼 실행 파일의 실경로(`/opt/homebrew/bin/gadak`
은 `/Applications/Gadak.app` 안으로 들어가는 링크라, 풀지 않으면 빌드가 아니라 링크를
가리킵니다)와 거기 붙은 서명의 종류(게시본은 `developer-id`, 로컬 툴체인이 서명한 것은
`adhoc`)입니다. 이 둘이 '배포본이 망가졌다' 와 '배포본을 실행하고 있지 않다' 를 가릅니다.
실측에서 brew 는 8월에 설치한 cask 라고 말했는데 그 아래 앱은 로컬 빌드로 덮여 있었고,
뒤이은 Gatekeeper 거절은 게시된 dmg 를 상대로 제보됐습니다 — 그 dmg 는 공증돼 있고
멀쩡합니다 ([GDK-1798], [GDK-1794]). 그리고 캐시가 낡았다는 사실이 이제 에이전트가 읽는
자리에 실립니다. 낡음 판정 — 첫 sync 가 도는 중인지, 지난 sync 가 실패했는지, 마지막
sync 로부터 한 시간이 지났는지 — 을 CLI 밖으로 꺼내 두 호출자가 같이 쓰는 소유자 하나로
만들었고, MCP 읽기 툴은 그것을 어떤 MCP 호스트도 못 보는 stderr 대신 결과에 덧붙입니다.
기존 페이로드는 바이트 그대로이고, 신선한 캐시에는 아무것도 붙지 않습니다 ([GDK-599]). gadak 이 설치해 주는 Raycast 확장도 자기 몫의 답 둘을 갖게
됐습니다. "gadak 이 설치돼 있지 않습니다" 화면이 클립보드에 넣던 것은 `brew install
midagedev/tap/gadak` 였는데 앱은 그렇게 설치하지 않습니다 — cask 입니다. 그리고 아무것도
맞지 않은 검색은 화면에 아무것도 그리지 않아서, 캐시에 답이 없는 것과 확장이 죽은 것이
같아 보였습니다. 이제 둘 다 자기가 무슨 상태인지 말하고, 스토어 리스팅은 심사가 요구한
스캐폴딩을 갖췄습니다 ([GDK-1800]).
