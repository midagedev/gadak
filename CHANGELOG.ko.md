...
**서식 있는 본문이 마크다운 편집을 살아서 지나가고, 툴바는 문 하나입니다.** 편집
원본이 마크다운이 말하지 못하는 노드마다 — 패널·멘션·이미지·색 런 — 자리표시자를
들고, 쓰기는 각 노드를 마커 자리에 되돌려 놓습니다. 그래서 셸 있는 에이전트의
`gadak issue` → 편집 → `edit -m -` 은 무손실이고, 문서 자체를 다루려는 에이전트에게는
`--adf-file` 이 raw 경로입니다. 목록 툴바는 레이아웃·정렬·컬럼·"뷰로 저장"을 보기
설정 메뉴 하나로 접었습니다. 그 아래 버그 라운드: Built-in close 의 resolution, edit
플래그의 batch 표기, 아바타 이니셜, 코드 안에 인용한 마커, 커널이 받았다면서 적용하지
않은 리사이즈.
...
- **커널이 받지 않은 터미널 리사이즈는 믿지 않고 다시 보낸다.** 데스크톱 리사이즈
  테스트가 일주일 동안 CI 에서 플레이키였다 — 세션은 132×43 을 적용했다고 믿는데 자식은
  24 80 을 답했다. 이번 릴리스에 넣은 읽기-되돌림이 현장을 잡았다: TIOCSWINSZ 는 성공을
  돌려줬는데 같은 master 의 TIOCGWINSZ 는 여전히 80×24 였다(macOS arm64 러너). 이제
  리사이즈의 소유자가 매번 설정 뒤 크기를 되읽어 커널이 동의할 때까지 한도 안에서 다시
  보내고, 끝내 안 잡히는 크기는 셸과 어긋난 pane 이 아니라 에러로 드러난다
  ([GDK-1192]).
...
- **목록 툴바의 보기 설정 메뉴 하나**(슬라이더 글리프)가 컨트롤 셋을 대신한다: 레이아웃
  (목록 ⇄ 보드)·정렬과 방향·노출 컬럼·"뷰로 저장"이 한 패널이다 — 원래 한 객체(뷰의
  display)였다. 보드 토글과 컬럼 메뉴는 손가락 하나 거리에서 같은 세 기둥 글리프를 달아
  컨트롤 하나로 읽혔다. 패널은 여러 항목을 고치는 동안 열려 있고, 브레이크다운 축은
  그것이 다스리는 칩 옆 스트립에 그대로 남는다. 레이아웃은 한 클릭이 아니라 두 클릭이
  됐다 ([GDK-1391]).
...
- **셸에 이름이 있다.** 새 셸은 16진수 여덟 자가 아니라 "셸 3" 이다: 서버가 세션마다
  ...
  기본 이름 — 행에서는 이름이 이슈 키보다 앞서고 키는 옆에 남는다. 카드↔셸 조인은
  라벨이 아니라 키를 읽기 때문이다 ([GDK-1195]). 그리고 이슈 헤더에 셸 동사가 생겼다:
  셸이 없는 이슈에서 누르면 도크가 열리며 그 이슈에 첫 프롬프트부터 묶인 새 셸이
  만들어지고 — 전에는 `gadak claim` 만이 만들 수 있던 바인딩 — 셸이 있는 이슈에서
  누르면 그 셸을 보고 있다 ([GDK-1388]).

- **상세가 모든 본문을 마크다운으로 읽고, 편집기에 미리보기가 생겼다.** 본문이
  어떻게 보일지는 이제 서버가 정한다: 저장된 ADF 가 친 텍스트뿐인 설명·코멘트 —
  옛 `-m`, 옮겨온 이슈 — 는 원래 그것이던 마크다운으로 읽히므로, 컷오버 이후 회색
  벽 하나로 보이던 844건이 데이터를 건드리지 않고 헤딩·목록·문단으로 렌더된다;
  Linear 본문(wire 가 마크다운)도 같은 길로 렌더된다. Jira 편집기가 서식을 준
  본문은 그대로 보인다. 설명 편집기는 서버가 건네는 마크다운으로 열리고, 서식
  손실 배너는 헤딩마다 거절하는 대신 저장이 실제로 지울 것(패널·인라인 이미지·
  멘션)만 이름 부르며, 설명 편집기와 코멘트 작성기 둘 다 쓰기 / 미리보기 탭이
  있다 — 미리보기는 저장이 남길 문서 그대로를 같은 서버 변환기로 렌더한 것이라
  웹은 자기 마크다운 파서를 갖지 않는다 ([GDK-1385]).
