...
- 터미널의 진입점과 배치: 커맨드 팔레트에 자리를 잡았고(`Ctrl+\`` 를 함께
  ...
  바꾸던 방식 그대로 바꿉니다 ([GDK-864], [GDK-853]).
...
- 토큰 하나를 설정하다 나머지를 잃는 일이 없어집니다: `ui.tokens.colors` /
  ...
  관계 문장을 나열합니다. 이제 토큰 하나는 자기 스칼라 경로를 갖습니다 —
  `gadak config set ui.tokens.type.terminal 15px`, JSON도 따옴표도 없이. 이름은
  카탈로그에서 해석되므로 유효 경로 목록은 토큰마다 한 줄이 아니라
  `ui.tokens.<axis>.<name>` 네 줄만 늘고, 모르는 이름은 오타로 저장되는 대신
  발견 명령을 알려주며 거절됩니다 ([GDK-852], [GDK-853]).
