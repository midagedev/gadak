...
- **스프린트가 이슈에 붙은 문자열 셋이 아니라 개체가 됐습니다.** 지금까지
  ...
  기간도 소속 보드도 어디에도 없었습니다. 캐시에 `sprints`·`boards` 테이블이
  생겼고 Agile API에서 채웁니다. Cloud와 Server가 같은 형태로 답하는 유일한
...
- **actor 꼬리말이 켜져 있으면 선택 기능이 사라졌습니다.** 에이전트 서명을
  ...
  참조, 스프린트가 전부 "이 origin이 무엇을 할 수 있나"를 묻는 코드에 보이지
  않았습니다. 이제 래퍼 너머를 봅니다. ([GDK-1655])
...
- **`gadak mcp install claude-desktop`가 Claude Desktop에 등록합니다. 그리고
  ...
  것이라 Claude Desktop은 읽지 않습니다. 따라 한 Desktop 사용자는 아무것도
  ...
  파일은 덮어쓰지 않고 거절, `--dry-run`은 쓸 내용만 출력). `claude` CLI를
...
- **앞문이 빠르다는 말보다 무엇인지를 먼저 말합니다.** 세 언어의 랜딩 제목이
  ...
  토큰을 건네기 *전에* 알아야 할 사실로 바뀌었습니다: Atlassian Cloud만
  ...
  여전히 Jira에 묻는 읽기 넷, 에이전트는 읽은 것을 자기 모델로 보냄. Rovo
  비교표는 연출된 양보 행 대신 실제 비용(로컬 바이너리와 첫 동기화)을
  얻었고, "API 8 페이지" 는 JQL의 법칙이 아니라 이번 측정의 숫자로 표기되고,
  귀속 주장은 코드가 하는 만큼으로 줄었으며(댓글과 만든 이슈에 에이전트 이름,
  PR은 없음), 모든 판본이 써 보고 어땠는지 묻는 절과 공개 이슈에 붙이지 말
...
- 마크다운으로 쓰고 ADF로 저장한다. `create -m`, `edit -m`, `comment`, page 동사, 웹
  편집기가 마크다운(제목, 목록, 코드, 표, 링크)을 받아 Jira나 Built-in 트래커의 형식으로
  저장한다 ([결정 0012](docs/decisions/0012-markdown-is-the-editing-source.md), [GDK-1384],
  [GDK-1383]). 상세 패널이 모든 본문을 마크다운으로 렌더하므로, 컷오버 이후 회색 덩어리로
  보이던 이슈들이 다시 블록으로 읽히고, 편집기에 Write / Preview가 생겼다 ([GDK-1385]).
  Linear에도 같은 서식이 그대로 간다 ([GDK-1386]).
- 서식 있는 본문을 잃는 것 없이 편집한다. `gadak issue`가 마크다운을 찍고 ([GDK-1394])
  마크다운이 표현하지 못하는 노드(패널, 멘션, 이미지, 색 런)마다 자리표시자를 세우며,
  `edit -m -`은 각 노드를 마커 자리에 되돌려 놓는다. 마커를 지우면 그 노드가 삭제되고,
  쓰기가 무엇을 지웠는지 알려 준다. 낡은 마커는 거절한다 ([GDK-1396]). `edit --adf-file`과
  `comment --adf-file`은 ADF 문서를 그대로 보낸다 ([GDK-1395]). `migrate --from`은 본문을
  평문이 아니라 ADF로 옮긴다 ([GDK-1382]).
- 목록 툴바에서 레이아웃, 정렬, 컬럼, "뷰로 저장"이 보기 설정 메뉴 하나로 들어갔다. 보드
  버튼과 컬럼 버튼이 같은 글리프를 달고 있었기 때문이다 ([GDK-1391]). 상태 브레이크다운
  축은 어느 origin에서나 "상태"라고 부르고, 순서는 두 번째다 ([GDK-1390]).
- 터미널에서 셸 이름이 16진수 여덟 자리가 아니라 "셸 3"이고, F2나 더블클릭으로 바꿀 수
  있으며, 이슈 헤더에서 그 이슈의 셸을 열거나 포커스한다 ([GDK-1387], [GDK-1195],
  [GDK-1388]).
- Windows에서는 `build-windows.ps1 --msix`가 Microsoft Store 패키지를 만든다. zip은 그대로
  남는다 ([GDK-1380]).
- 수정한 것. 새 Built-in 워크스페이스에서 `close --resolution`이 된다 ([GDK-1347]).
  `edit --batch`가 `label`, `component`, `fix_version`을 받는다 ([GDK-1259]). 아바타
  이니셜이 구두점을 건너뛴다 ([GDK-1351]). 코드 안에 인용한 자리표시자 마커는 텍스트로
  남는다 ([GDK-1398]). 커널이 받지 않은 터미널 리사이즈는 잡힐 때까지 다시 보낸다. CI
  플레이크의 원인이었다 ([GDK-1192]).
...
맨 위에 워크스페이스 행, 조용한 섹션, 한 줄 푸터가 있습니다([GDK-1335]).
리스트 툴바는 뷰를 좁히기 전까지 한 줄이고, 행은 36px, 오래된 이슈의 나이는
상자 대신 숫자로 보이며, 팔레트로 들어가는 문은 하나입니다([GDK-1336]). 문서,
스페이스, 방문 기록, 대시보드, 피드 같은 모든 열 화면이 헤더 띠 하나를
공유합니다([GDK-1339]). 상세는 바뀌는 것만 남긴 칩 행 아래의 속성 목록
하나이고([GDK-1337]), 본문에는 타입 스케일과 넓게 읽기 토글이
...
**뒤로가기가 어디서나 되고, 첫 인바운드 라운드를 전부 담았습니다.**
@woojing 님이 다섯 건을 제보해 주셨고(디스커션 #80, 이슈 #85), 모두 이
릴리스에 들어갔습니다. 감사합니다. 링크된 이슈, 사람, 뷰, 검색어 등 모든
장소 이동이 history를 거치므로 브라우저의 뒤로 버튼이 말 그대로
동작합니다([GDK-1296]). 상세의 링크 복사는 origin 페이지를 먼저
붙이고([GDK-1290]), 설정 대화상자는 탭을 바꿔도 한 폭을 지키고([GDK-1291]),
전체화면 창은 Dock을 클릭해도 전체화면에 머물고([GDK-1294]), 이 컴퓨터에 없는
워크스페이스를 가리키는 `gadak://` 링크는 앱을 404 페이지로 바꾸는 대신
대화상자로 거절됩니다([GDK-1309]). 그 과정에서 이슈 링크는 자기 origin에서
열리게 됐습니다. 트래커별 분기 하나뿐이고, 서로 넘나드는 폴백은
없습니다([GDK-1149], [GDK-1308]).
...
없었습니다([GDK-1361]). 이름 붙은 워크스페이스의 터미널에 친 쓰기가 더는
`warning: workspace: … (from GADAK_WORKSPACE)`로 시작하지 않습니다. 그 변수는
serve가 창의 워크스페이스를 알리려고 넣은 것이기 때문입니다([GDK-1362]). 신선도 칩이
...
내보낸다. 이슈, 코멘트, 전체 이력, 링크, 첨부 바이트, 위키 페이지가 옮겨지고,
끝에 원본과 이전본의 대조표가 나온다. 대조표에는 파생 컬럼도 들어간다. reopen
횟수와 epic 키는 이전된 이력에서 다시 계산돼야 하므로, 그 두 줄이 같다는 것이
진짜 증거다. 손실은 침묵하지 않고 보고된다. 본문은 평문으로 옮겨지고 코드
블록, 미디어, 표가 몇 개 평탄화됐는지 적히며, 이전 대상 밖을 가리키는 링크와
부모는 누락으로 집계된다. 이 프로젝트 자신의 백로그로 실측했다. 이슈 1,268건,
이력 3,811행, 첨부 26/26, 전 행 일치 ([GDK-1264]).

그 첫 전량 export가 어렵게 배운 것 둘을 남겼다. 시드 문서는 JSON으로 나간다.
YAML emitter가 실제 트래커 본문에서 자기 파서도 거부하는 블록 스칼라를 쓰기
...
work/NMA-9`는 포인터를 기록한다. 내 origin에 Jira remote issue link로
저장되며, 가리킨 워크스페이스에는 아무것도 쓰지 않는다. 팀 티켓에 관한 개인
메모가 개인의 것으로 남는 이유다. 값어치는 반대쪽에 있다. `gadak ref STD-1
--list`와 앱의 참조 섹션이 대상의 **현재** 상태, 담당자, 요약을 보여주는데, 이
컴퓨터에 있는 그 워크스페이스의 미러에서 읽는다. 네트워크 호출도, 탭 전환도
없다. 이 컴퓨터가 미러하지 않는 대상도 목록에 남고, 미러가 없다고 말해 줄
뿐이다 ([GDK-1032]).
...
그 외에도 에이전트 스킬이 부딪힌 벽을 스스로 진단하고, 마찰을 조용히 우회하는
...
**내 테마는 내 것입니다. 검증은 거절하는 대신 경고하고 저장합니다.** 대비, 색차,
...
짚고, 타입 줄은 함께 움직여야 하는 글자 크기 단계 전체를 찍습니다. 어떤
테마에서든 빠져나오는 길은 언제나 CLI 한 줄입니다 ([GDK-856], [GDK-857], [GDK-858]).

**대시보드의 링크가 이슈를 열 수 있습니다.** 대시보드에서 이슈, 저장된 뷰,
필터된 목록, 검색 등 앱의 어느 경로로든 이동할 수 있고, 외부 링크는 대시보드를
...
바로 거기였습니다. `issues_full`에 `description_text`가 생겼고, standalone
...
들어왔습니다. 워크스페이스에는 일부러 아직 연결하지 않았고, 연결은
...
확장이 로컬 앱처럼 느껴지는 것은 그 덕분입니다 ([GDK-117], [GDK-166],
[GDK-170]).
...
하한을 통과합니다. 상태 색은 정상 시각과 색각 이상에서 모두 구분되고, 성공과
실패를 색만으로
...
신뢰에 관한 릴리스입니다. 조용히 실패하는 대신 크게 실패하는 표면, 코드와
맞는 문서, 형용사 대신 실측한 숫자를 담았습니다.
...
저장된 뷰, watch, 즐겨찾기가 그것입니다. 자격 증명도 사이트 URL도 담지 않습니다.
...
즉 이미 있는 것을 좁히는 일을 계속합니다.

**기록은 미러 옆의 파일입니다.** 이슈·문서·검색이 `~/.gadak/local.db`의 한
타임라인에 있으므로, 미러를 버려도 기록은 남고, 에이전트는 내가 방문한
...
`gadak views`가 그 필터를 나열하고, 표시하고, 열고, 저장합니다.
...
외양과 개명의 릴리스입니다. gadak은 가닥입니다. 코팅 없는 종이, 먹, 쪽빛 실
한 올입니다.
...
- Gadak.app, macOS 데스크톱 앱입니다. 웹 UI가 서명·공증된 자기 창에 있고, 로컬
