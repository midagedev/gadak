...
- 에이전트가 저작하는 대시보드는 HTML 문서 하나와 등록된 데이터소스들로
  ...
  미러 변경은 2초 안에 데이터를 다시 밀며, uPlot은 고정된 같은-오리진
  화이트리스트로 임베디되어 차트가 CDN도 CSP 확장도 필요로 하지
  않습니다 ([GDK-781], [GDK-782], [GDK-792], [GDK-793]).
...
- uPlot을 넘선 라이브러리는 *사용자가* 한 번 내려받은 것이면 무엇이든
  ...
  defer 스크립트로 주입하며 script-src에 로컬 libs 경로만 추가합니다 —
  CSP는 여전히 외부 호스트를 절대 담지 않고, 선언 없는 대시보드는
