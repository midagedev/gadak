...
단언합니다. 걸린 줄은 어떤 형이 맞았는지 이름을 답니다. PEM 은 의도적으로
빼 뒀고 — PEM 헤더를 담는 것이 존재 이유인 테스트 벡터가 둘 있습니다 — 그 결정을
커밋 메시지가 아니라 스크립트에 적고 테스트로 고정했습니다 ([GDK-1110]). 설정 창은 레지스트리가 한 번도 가져 본 적
