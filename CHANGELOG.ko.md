...
**본문은 마크다운이고, 서식 있는 본문도 왕복을 살아서 지나갑니다.**

- 마크다운으로 쓰고 ADF 로 저장. `create -m`·`edit -m`·`comment`·page 동사·웹 편집기가
  마크다운(제목·목록·코드·표·링크)을 받아 Jira 나 Built-in 트래커의 형식으로 저장한다
  ([결정 0012](docs/decisions/0012-markdown-is-the-editing-source.md), [GDK-1384],
  [GDK-1383]). 상세 패널이 모든 본문을 마크다운으로 렌더해 컷오버 이후 회색 벽으로
  보이던 이슈들이 다시 블록으로 읽히고, 편집기에 Write / Preview 가 생겼다 ([GDK-1385]).
  Linear 에도 같은 서식이 그대로 간다 ([GDK-1386]).
- 서식 있는 본문의 무손실 편집. `gadak issue` 가 마크다운을 찍고 ([GDK-1394]) 마크다운이
  말하지 못하는 노드(패널·멘션·이미지·색 런)마다 자리표시자를 세우며, `edit -m -` 은 각
  노드를 마커 자리에 되돌려 놓는다. 마커를 지우면 그 노드가 삭제되고 쓰기가 그렇게
  말한다. 낡은 마커는 거절한다 ([GDK-1396]). `edit --adf-file`·`comment --adf-file` 은
  ADF 문서를 그대로 보낸다 ([GDK-1395]). `migrate --from` 은 본문을 평문이 아니라 ADF 로
  옮긴다 ([GDK-1382]).
- 목록 툴바: 레이아웃·정렬·컬럼·"뷰로 저장"이 보기 설정 메뉴 하나로 — 보드 버튼과 컬럼
  버튼이 같은 글리프를 달고 있었다 ([GDK-1391]). 상태 브레이크다운 축은 어느 origin 에서나
  "상태", 두 번째 ([GDK-1390]).
- 터미널: 셸 이름이 16진수 여덟 자리가 아니라 "셸 3"이고, F2 나 더블클릭으로 바꿀 수
  있으며, 이슈 헤더에서 그 이슈의 셸을 열거나 포커스한다 ([GDK-1387], [GDK-1195],
  [GDK-1388]).
- Windows: `build-windows.ps1 --msix` 가 Microsoft Store 패키지를 만든다. zip 은 그대로
  ([GDK-1380]).
- 수정: 새 Built-in 워크스페이스에서 `close --resolution` 이 된다 ([GDK-1347]);
  `edit --batch` 가 `label`·`component`·`fix_version` 을 받는다 ([GDK-1259]); 아바타
  이니셜이 구두점을 건너뛴다 ([GDK-1351]); 코드 안에 인용한 자리표시자 마커는 텍스트다
  ([GDK-1398]); 커널이 받지 않은 터미널 리사이즈를 잡힐 때까지 다시 보낸다 — CI
  플레이크 ([GDK-1192]).
