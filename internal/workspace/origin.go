...
// ensureOrigin binds this process's embedded built-in origin onto a
...
func (r *Registry) ensureOrigin(name string, e *Entry) {
	if r == nil || e == nil || e.Cfg == nil || !e.Cfg.HasBuiltInOrigin() {
		return
	}
	...
	cfg := e.Cfg
	h, err := origin.BuiltInHandler(cfg)
	if err != nil {
		if logf != nil {
