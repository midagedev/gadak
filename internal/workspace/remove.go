...
// the next move: the root workspace is the home itself, a built-in persist
...
// config is a refusal, not a guess: if this might be builtIn, its persist
...
// NeedsDestroyOriginError: built-in with an existing persist, but the
...
// Built-in picks the wording that says there is no origin data to protect
...
type RemoveResult struct {
	...
	// (empty when none), Built-in the pre-removal kind reading.
	Dir     string
	Persist string
	BuiltIn bool
...
func Remove(name string, yes, destroyOrigin bool, cmdHint string) (RemoveResult, error) {
	...
	builtIn := cfg.HasBuiltInOrigin()
	persist := ""
	if builtIn {
		persist = existingPersist(dir)
	}

	if builtIn && persist != "" && !destroyOrigin {
		return RemoveResult{}, &NeedsDestroyOriginError{Name: name, Persist: persist, CmdHint: cmdHint}
	}
	if !yes {
		return RemoveResult{}, &NeedsYesError{Name: name, CmdHint: cmdHint, BuiltIn: builtIn}
	}
