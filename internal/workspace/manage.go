...
// originbind.SeedBuiltIn, the same core `gadak init --standalone` and
...
// spelled by the caller, not defaulted: built-in seeds a local issuetap
...
// CreateHandler answers POST /api/v1/workspaces by seeding a built-in
// workspace profile or registering a paired one (kind "paired" + offer).
// The built-in seed is the shared one: config write, default issue
// type, mirror fill. A fill that fails is logged, not failed
// (SeedBuiltIn contract — the workspace exists and writes work; the
...
func CreateHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// reports kind connected (WorkspaceKind — only built-in is its
		...
		// initBuiltIn does the same open/close).
		...
		fillErr, err := originbind.SeedBuiltIn(cfg, in.Projects, nil, openMirror)
		if err != nil {
		...
		if err := origin.CloseBuiltIn(cfg); err != nil {
