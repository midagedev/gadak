...
// pairflowHome stands up a temp GADAK_HOME with a built-in workspace —
...
// a built-in home, the _home routing token exists and authorizes. First
...
func TestMintDeviceContract(t *testing.T) {
	...
	if res.HomeRouting != HomeRoutingMinted {
		t.Fatalf("built-in mint reports %q, want minted", res.HomeRouting)
	}
...
// caller prints, refuses a non-built-in workspace, and — like the device
