...
func defaultKeys() keyMap {
	return keyMap{
		...
		// 1–4 are context-bound: list status · feed focus · docs list axis.
		TabAll:        key.NewBinding(key.WithKeys("1"), key.WithHelp("1", "all · feed all · docs updated")),
		TabOpen:       key.NewBinding(key.WithKeys("2"), key.WithHelp("2", "open · feed assignee · docs by author")),
		TabInProgress: key.NewBinding(key.WithKeys("3"), key.WithHelp("3", "in progress · feed reporter · docs spaces")),
		TabDone:       key.NewBinding(key.WithKeys("4"), key.WithHelp("4", "done · feed mention")),
