...
// built-in (issuetap speaks the Jira API) satisfy it via jiraWriter
...
// fixVersions add {"name": token}. True for issuetap (builtIn in-process,
...
// issuetap-backed origin (builtIn / paired) — a Cloud site would show
...
// hasRemoteLinks reports an issuetap-backed origin — built-in
...
// Linear "MID-5" from a Jira "MID-5". Jira and built-in rows share the
