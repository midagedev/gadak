...
// helpers come from origin_rest_test.go: builtInServer builds a real
// built-in workspace under a temp GADAK_HOME, get() drives the Handler.
//
...
//
// The in-process Basic credential ("built-in:built-in") is what today's
...
func basicBuiltIn(t *testing.T) string {
	t.Helper()
...
func TestPairingGateRejectsWithoutValidBearer(t *testing.T) {
	...
	for name, headers := range map[string]map[string]string{
		...
		"basic only": {"Authorization": basicBuiltIn(t)},
		"no auth":    nil,
