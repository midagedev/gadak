...
func SyncIssue(ctx context.Context, cfg *config.Config, db *store.DB, key string, opts Options) error {
	...
	// Write-through can run before the first full sync (builtIn create is
	...
	found := false
	err = c.Search(ctx, fmt.Sprintf("key = %q", key), appendSprintField(fieldList(cfg, false), agile), true, func(issues []jira.Issue) error {
