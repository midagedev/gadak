...
func Hash(f Filter, d Display) string {
	...
	// Negation twins (GDK-771): include short key + 'n', same derivation as
	// the web's NEGATION_KEY. A twin missing here reads as "nothing applied"
	// in views save — the hash, not Applied, is that gate.
	addList("scn", f.StatusCategoryNot)
	addList("stn", f.StatusNot)
	addList("asn", f.AssigneeEmailNot)
	addList("rpn", f.ReporterEmailNot)
	addList("grn", f.TeamGroupNot)
	addList("lbn", f.LabelsNot)
	addList("prn", f.PriorityNot)
	addList("svn", f.SeverityNot)
	addList("tyn", f.IssueTypeNot)
	addList("con", f.ComponentsNot)
	addList("fxn", f.FixVersionsNot)
	addList("qrn", f.QARunNot)
	addList("qsn", f.QASuiteNot)
	addList("qin", f.QAImpactNot)
	addList("dsn", f.DeployStateNot)
	addList("ks", f.Keys)
	addList("pk", f.Parent)
