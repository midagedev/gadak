...
// init or onboarding connect refuses to take over a built-in workspace
...
// replaceMessage is the former builtInReplaceMessage. Wording is a
...
// RefuseIfOpen stops a CLI built-in→connected conversion while another
...
// DropBuiltInProjection is the conversion cleanup both CLI init and HTTP
...
func DropBuiltInProjection(cfg *config.Config, db *store.DB) (store.OriginReset, error) {
	if cfg != nil {
		cfg.Confluence = nil
	}
	if db == nil {
		return store.OriginReset{}, fmt.Errorf("drop built-in mirror: nil store")
	}
	reset, err := db.ResetForNewOrigin(context.Background(), []string{"jira", "confluence"})
	if err != nil {
		return store.OriginReset{}, fmt.Errorf("drop built-in mirror: %w", err)
	}
	return reset, nil
}

// RefuseReplace stops a connected init / onboarding connect from silently
// changing which origin owns a built-in workspace that holds locally
// originated data. An empty built-in workspace (tried it, nothing
...
func RefuseReplace(cfg *config.Config, replace bool) error {
	if cfg == nil || !cfg.HasBuiltInOrigin() || replace {
		return nil
	}
	n, persist, err := LocalData(cfg)
	if err != nil {
		return fmt.Errorf("cannot replace built-in workspace: %w", err)
	}
	if n == 0 {
		return nil
	}
...
// ClearBuiltIn is the single owner of "origin is now a Jira site, so
// this workspace is no longer built-in". Reached only after RefuseReplace
// (or an explicit replace opt-in). Empty built-in workspaces take this
// path too: once connected, Kind must be cleared.
func ClearBuiltIn(next *config.Config) {
	if next == nil {
		return
	}
	next.Kind = ""
...
// LocalData reports how much locally originated data this built-in
