...
// seedBuiltInIssue puts one issue in the built-in mirror under the key a
...
func seedBuiltInIssue(t *testing.T, db *store.DB) {
	t.Helper()
...
func TestConversionLeavesNoRowBoundToTheOldOrigin(t *testing.T) {
	...
	if _, err := db.RecordRecent(ctx, "assignee", "built-in-acct-1"); err != nil {
		t.Fatal(err)
	}
	...
	reset, err := DropBuiltInProjection(cfg, db)
	if err != nil {
		t.Fatal(err)
	}
