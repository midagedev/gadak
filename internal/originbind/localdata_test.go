...
func builtInHome(t *testing.T) *config.Config {
	t.Helper()
	home := t.TempDir()
...
// TestLocalDataCountsPagesWhenNoIssues is GDK-417: a wiki-only built-in
...
func TestRefuseReplaceBlocksPagesOnlyWorkspace(t *testing.T) {
	...
	if err == nil {
		t.Fatal("pages-only built-in must refuse a connected init")
	}
...
`)
	...
	if err := RefuseReplace(cfg, false); err == nil {
		t.Fatal("yaml-only built-in with issues must refuse a connected init")
	}
}
