...
How a coding agent is meant to read the mirror. This is half the reason gadak
...
run a shell command can already use gadak at full power, and the schema in
...
`watermark` is recent. `gadak status --json` prints the same information for
...
run a shell should keep using `gadak sql` / `gadak issue` / `gadak search` — MCP is
...
- **Do not depend on `issues.raw`.** It is an escape hatch for fields not yet
  mapped, and its shape follows Jira's API, not gadak's contract.
