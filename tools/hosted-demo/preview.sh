#!/usr/bin/env bash
# Serve dist/hosted at http://127.0.0.1:4173/gadak/ so the first frame
# can be checked at 390×844 (phone / in-app) or any desktop width.
set -euo pipefail
...
if ! grep -q 'data-testid="hosted-first-frame"' "$ROOT/dist/hosted/index.html"; then
  echo "hosted-demo: index.html has no first frame — rebuild with node tools/hosted-demo/build.mjs" >&2
  exit 1
fi
rm -rf "$ROOT/dist/pages"
mkdir -p "$ROOT/dist/pages/gadak"
cp -R "$ROOT/dist/hosted/." "$ROOT/dist/pages/gadak/"
echo "hosted-demo: http://127.0.0.1:4173/gadak/  (390×844 to check the first frame)" >&2
