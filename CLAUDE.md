...
- **CHANGELOG는 다시 쓸 수 있다 — 서사를 위해서라면 과감하게 정리한다**
  ...
  지워도 된다. 불변은 넷뿐이다: ① **릴리스 헤딩과 그 순서**(사이트가 그것으로
  점프 목록을 만들고 doc-checks #27이 세 판본을 대조한다) ② **모든 `GDK-nnn`
  키가 원래 릴리스 안에 남는다**(주제로 합치면 그 문단이 키를 다 든다) ③
  **숫자·명령·경로·플래그·코드·링크** ④ 없던 일을 새로 쓰지 않는다. 재는 것은
  `bash tools/changelog-preserve.sh <파일> [ref]` — 헤딩·키·숫자가 움직이면 진짜
  실패이고, 구조를 바꾼 라운드는 코드 스팬·링크 축이 정당하게 빨갛다(그 출력을
  읽고 보고에 넣는다). 판본은 셋이고(`CHANGELOG.md`·`.ko.md`·`.ja.md`) 여기서는
