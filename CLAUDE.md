...
- **CHANGELOG는 다시 쓸 수 있다 — 서사를 위해서라면 과감하게 정리한다**
  (사용자 지시 2026-09-09 "히스토리 수정금지 아니야 서사를 위해서라면 과감하게
  정리하고 정돈해도 돼"). 2026-08-20의 "링크화만 예외"와 그날 오전의
  "문장만 다듬어도 된다"를 둘 다 흡수한 개정이다. 항목을 합치고, 순서를 바꾸고,
  줄이고, 키 없는 항목은 지워도 된다. 불변은 넷뿐이다: ① **릴리스 헤딩과 그
  순서**(사이트가 그것으로 점프 목록을 만들고 doc-checks #27이 세 판본을
  대조한다) ② **모든 `GDK-nnn` 키가 원래 릴리스 안에 남는다**(항목을 합치면
  합쳐진 항목이 양쪽 키를 다 든다) ③ **숫자·명령·경로·플래그·코드·링크** ④
  없던 일을 새로 쓰지 않는다. 재는 것은 `bash tools/changelog-preserve.sh
  <파일> [ref]` — 헤딩·키·숫자가 움직이면 진짜 실패이고, 구조를 바꾼 라운드는
  코드 스팬·링크 축이 정당하게 빨갛다(그 출력을 읽고 보고에 넣는다). 판본은
  셋이고(`CHANGELOG.md`·`.ko.md`·`.ja.md`) 여기서는 병렬 판본이 아니라 같은
  역사의 번역이다 — **영문 구조를 바꾸면 ko·ja도 같은 구조로 따라가야 한다**
  (doc-checks #27이 절별 키 집합 일치를 잰다). `GDK-nnn`을 공개 백로그
  (`…/gadak/backlog/#/?ks=<KEY>`)로 참조 스타일 링크로 잇는 것은 여전히 기본값이다.
