 ...
 * a built-in workspace, see it listed, then walk the two-step removal
 * (the probe DELETE surfaces the server's own refusal wording — which for a
 * built-in workspace is the only-copy warning with the persist path —
...
test.describe('workspaces settings tab', () => {
  test('create, list, and two-step remove of a built-in workspace', async ({ page, request }) => {
    ...
    // refusal into the dialog — for a built-in workspace that wording is
...
test.describe('workspaces settings tab — register remote (GDK-1099)', () => {
  ...
  test('register remote: refusals render inline, success adds a connected row and clears the code', async ({ page }) => {
    ...
    // the built-in test filters the protocol's 400/409 above.
