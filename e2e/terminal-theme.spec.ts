...
/** One named slot of the live terminal's applied theme (GDK-1358 ANSI,
 *  GDK-1359 cursor), and the same token as the host computes it. */
async function appliedSlot(page: Page, slot: string, token: string) {
  return page.evaluate(
    ({ slot, token }) => {
      const t = (window as unknown as { __gadakTerm?: { options: { theme?: Record<string, string> } } })
        .__gadakTerm
      if (!t?.options.theme) throw new Error('no live theme')
      const host = document.querySelector('[data-testid="terminal-pane"] .terminal-host')!
      return {
        applied: t.options.theme[slot] ?? '',
        host: getComputedStyle(host).getPropertyValue(token).trim(),
      }
    },
    { slot, token },
  )
