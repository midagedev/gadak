...
/**
 * Sprint promo recording — the board scoped to a sprint, the backlog, and back.
 * Separate from e2e/demo/playwright.config.ts so `make media-web` cannot
 * pick up this video.webm by accident.
 *
 * Run via `make media-sprint` (sets GADAK_MEDIA=1).
 */
export default defineConfig({
  testDir: '.',
  testMatch: 'sprint-demo.spec.ts',
  fullyParallel: false,
