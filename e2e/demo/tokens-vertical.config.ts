...
/**
 * Tokens promo — vertical 1080×1350 (4:5) social cut.
 * Reuses tokens-demo.spec.ts; GADAK_PROMO_LAYOUT=vertical selects stacked
...
if (!process.env.GADAK_E2E_PORT) {
  throw new Error(
    'tokens-vertical.config.ts: set GADAK_E2E_PORT (record-vertical.sh uses 7890) so this does not collide with the landscape promo on 7888',
  )
}
...
export default defineConfig({
  ...
  outputDir: '../.tmp/test-results-tokens-vertical',
  use: {
