...
#                     # cameras watch one mirror. Needs no Claude login and
#                     # builds no credential drive: it writes its own PTY
#                     # env instead (see the block near the preconditions)
...
# --serve-only holds a port; nobody types Claude into a pane, so it has no
# reason to build the credential drive (a 0600 copy of this machine's
# logins) and no reason to leave one behind for the operator to clean. When
# the drive is not already there it writes the PTY's environment itself —
# the same contract the tapes' env carries, minus the credentials:
#
#   PS1       — a bare `$ `. Left to the default, /bin/sh on macOS prompts
#               `sh-3.2$`; that is the shell's version string, on camera, in
#               a film about a tool. Measured: a hand-rolled `env -i` serve
#               put exactly that in a phone take. $SHELL stays unset on
#               purpose — the fallback is /bin/sh (internal/term), and bash
#               in sh mode is the one invocation that does not print Apple's
#               three-line "the default interactive shell is now zsh" nag.
#               Setting SHELL=/bin/bash to be explicit put that nag in the
#               next take; the silencer below covers an inherited SHELL.
#   HOME, cwd — a throwaway under the hero root. The PTY opens in the
#               serve's $HOME (internal/server/terminal.go), and the
#               operator's real home is what MEDIA.md keeps out of a frame.
#   PATH      — gadak first: the phone's whole line is `gadak close STD-n`.
#
# The desk take proper still prepares the drive — it needs a Claude login.
if [[ -n "$SERVE_ONLY" && ! -x "$GADAK_BIN" ]]; then
  GADAK_BIN="$ROOT/e2e/.tmp/gadak"
  [[ -x "$GADAK_BIN" ]] || {
    echo "record-hero-desk: $GADAK_BIN missing — go build -o e2e/.tmp/gadak ./cmd/gadak" >&2
    exit 1
  }
  SKIP_PREPARE=1
  ENV_SH="$OUT/env-serve-only.sh"
  mkdir -p "$HERO_ROOT/agent" "$OUT"
  cat >"$ENV_SH" <<EOF
# Generated by record-hero-desk.sh --serve-only — do not edit.
export PS1='\$ '
export PROMPT_COMMAND=
export BASH_SILENCE_DEPRECATION_WARNING=1
unset SHELL
export HOME='$HERO_ROOT/agent'
...
cd '$HERO_ROOT/agent'
EOF
  echo "record-hero-desk: serve-only — credential-free PTY env at $ENV_SH"
fi
...
# Both patches below are about the agent that types in the desk pane, and
# --serve-only has no such agent (nor, on the credential-free path, an
# $AGENT_HOME to patch).
if [[ -z "$SERVE_ONLY" ]]; then
