 ...
 * ── The mirror ────────────────────────────────────────────────────────────
 *
 * The workspace is a built-in one (record-hero-desk.sh builds it:
 * `gadak init --local`, seed issues only, no real data — MEDIA.md).
 * Built-in is load-bearing here, not a convenience: the frozen demo home
 ...
 * transition an issue. The built-in origin is issuetap in-process, on
 ...
 * Progress / Done — the whole built-in transition graph, en locale), and
...
test.describe('hero desk demo', () => {
  ...
  test('desk bits 1+6: walk away mid-task, come back to it done', async ({ page }) => {
    ...
    // ── 1. The list at rest, on a real (seeded, builtIn) mirror. ──────
    await page.goto('/#/')
